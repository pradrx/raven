...
| --- | --- |
| `/raven-new <slug> <description>` | Generate a new question + tests pair. Validates inline (the agent writes a reference solution **in context**, runs it against every case, and **never writes it to disk**). Atomic rollback on failure. |
...
├── templates/                  # paste-able canonical shapes
